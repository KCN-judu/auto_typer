...
class CanMotorHal {
 public:
  ...
  bool clearPosition() const {
    const uint8_t command[] = {config_.motorAddress, 0x0A, 0x6D, 0x6B};
    return sendCommand(command, sizeof(command));
  }

  bool moveAbsolute(const CanCommandPlan& plan, int32_t targetSteps) const {
    const uint32_t magnitude = targetSteps >= 0 ? static_cast<uint32_t>(targetSteps)
                                                : static_cast<uint32_t>(-targetSteps);
    const uint8_t command[] = {
      config_.motorAddress,
      0xFD,
      static_cast<uint8_t>(targetSteps >= 0 ? MotorDirection::Cw : MotorDirection::Ccw),
  ...
  CanTestResult runConnectivityCheck(const CanCommandPlan& plan, uint32_t timeoutMs = 1500) {
    ...
    if (!controllerReady) {
      return summarizeCanTest(false, false, false, false, false, false, false, false, feedback);
    }
    ...
    delay(150);
    const bool zeroCommandSent = clearPosition();
    delay(150);
    const int32_t target = plan.direction == MotorDirection::Cw ? static_cast<int32_t>(plan.targetSteps)
                                                                : -static_cast<int32_t>(plan.targetSteps);
    const bool absoluteCommandSent = moveAbsolute(plan, target);
    delay(1500);
    const bool returnCommandSent = moveAbsolute(plan, 0);
    delay(1500);
